...
export class AgentToolService {
  ...
  async runTool(ctx: AppContext, systemId: string, toolName: z.infer<typeof ToolNameSchema>, input: Record<string, unknown>) {
    ...
    if (toolName === "list_templates") return starterTemplates.map((t) => ({ id: t.id, name: t.title, category: t.category }));
    if (toolName === "search_node_types") {
      const q = String(input.query ?? "").toLowerCase();
      return nodeLibraryCatalog.filter((n) => n.nodeType.toLowerCase().includes(q) || n.name.toLowerCase().includes(q)).slice(0, 12);
    }
    if (toolName === "get_node_type_details") return nodeLibraryCatalog.find((n) => n.nodeType === input.nodeTypeId) ?? null;
    ...
    if (toolName === "run_simulation_summary") {
      ...
      return simulateSystem(bundle.system as never, bundle.nodes as never, ports as never, bundle.pipes as never, {});
    }
