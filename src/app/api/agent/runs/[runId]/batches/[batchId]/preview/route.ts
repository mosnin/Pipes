...
export async function POST(request: Request, { params }: { params: Promise<{ runId: string; batchId: string }> }) {
  try {
    ...
    return NextResponse.json(success(await service.getBatchPreview(ctx, { runId, batchId, selectedDiffIds: body.selectedDiffIds, previewEnabled: body.previewEnabled })));
  } catch (error) {
