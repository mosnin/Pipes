...
export function serializeConvexBundle(bundle: any): string {
  ...
  const connections = bundle.pipes ?? [];
  ...
  return serializeLooperSchema({
    ...
    systems: [{
      ...
      nodeIds: nodes.map((n: any) => String(n._id)), portIds: nodes.flatMap((n: any) => n.portIds), pipeIds: connections.map((p: any) => String(p._id)), groupIds: [], annotationIds: [], commentIds: [], assetIds: [], snippetIds: [], subsystemNodeIds: []
    }],
    views: [],
    ...
    pipes: connections.map((p: any) => ({ id: String(p._id), systemId: String(p.systemId), fromPortId: p.fromPortId, toPortId: p.toPortId })),
    groups: [], annotations: [],
