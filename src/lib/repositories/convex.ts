  ...
  return {
    ...
    systems: {
      ...
      async delete(systemId) {
        const client = await getConvexHttpClient();
        await client.mutation((api as any).app.deleteSystem, { systemId: systemId as never });
      },
    ...
    agentBuilder: {
      ...
      async upsertRunReviewer(input) { const row = await (await getConvexHttpClient()).mutation((api as any).app.upsertRunReviewer, { ...input, runId: input.runId as never, workspaceId: input.workspaceId as never, systemId: input.systemId as never }); return { id: String(row._id), ...input }; },
      async listRunReviewers(input) { const rows = await (await getConvexHttpClient()).query((api as any).app.listRunReviewers, { runId: input.runId as never }); return rows.map((row: any) => ({ id: String(row._id), runId: String(row.runId), workspaceId: String(row.workspaceId), systemId: row.systemId ? String(row.systemId) : undefined, userId: row.userId, role: row.role, status: row.status, joinedAt: row.joinedAt, updatedAt: row.updatedAt })); },
      async upsertSharedRunVisibility(input) { const row = await (await getConvexHttpClient()).mutation((api as any).app.upsertSharedRunVisibility, { ...input, runId: input.runId as never, workspaceId: input.workspaceId as never, systemId: input.systemId as never }); return { id: String(row._id), ...input }; },
      async listSharedRunVisibility(input) { const rows = await (await getConvexHttpClient()).query((api as any).app.listSharedRunVisibility, { runId: input.runId as never }); return rows.map((row: any) => ({ id: String(row._id), runId: String(row.runId), workspaceId: String(row.workspaceId), systemId: row.systemId ? String(row.systemId) : undefined, userId: row.userId, mode: row.mode, currentStage: row.currentStage, viewedAt: row.viewedAt, updatedAt: row.updatedAt })); },
      async addReviewThread(input) { const row = await (await getConvexHttpClient()).mutation((api as any).app.addReviewThread, { ...input, runId: input.runId as never, workspaceId: input.workspaceId as never, systemId: input.systemId as never }); return { id: String(row._id), ...input }; },
      async listReviewThreads(input) { const rows = await (await getConvexHttpClient()).query((api as any).app.listReviewThreads, { runId: input.runId as never, targetType: input.targetType, targetId: input.targetId }); return rows.map((row: any) => ({ id: String(row._id), runId: String(row.runId), workspaceId: String(row.workspaceId), systemId: row.systemId ? String(row.systemId) : undefined, targetType: row.targetType, targetId: row.targetId, diffLineageRef: row.diffLineageRef, status: row.status, createdBy: row.createdBy, createdAt: row.createdAt, resolvedBy: row.resolvedBy, resolvedAt: row.resolvedAt })); },
      async resolveReviewThread(input) { await (await getConvexHttpClient()).mutation((api as any).app.resolveReviewThread, input as any); },
      async addReviewComment(input) { const row = await (await getConvexHttpClient()).mutation((api as any).app.addReviewComment, { ...input, runId: input.runId as never, workspaceId: input.workspaceId as never, systemId: input.systemId as never, threadId: input.threadId as never }); return { id: String(row._id), ...input }; },
      async listReviewComments(input) { const rows = await (await getConvexHttpClient()).query((api as any).app.listReviewComments, { runId: input.runId as never, threadId: input.threadId as never, targetType: input.targetType, targetId: input.targetId }); return rows.map((row: any) => ({ id: String(row._id), threadId: String(row.threadId), runId: String(row.runId), workspaceId: String(row.workspaceId), systemId: row.systemId ? String(row.systemId) : undefined, targetType: row.targetType, targetId: row.targetId, authorId: row.authorId, body: row.body, createdAt: row.createdAt })); },
      async addReviewDecisionRecord(input) { const row = await (await getConvexHttpClient()).mutation((api as any).app.addReviewDecisionRecord, { ...input, runId: input.runId as never, workspaceId: input.workspaceId as never, systemId: input.systemId as never, approvalRequestId: input.approvalRequestId as never, batchId: input.batchId as never }); return { id: String(row._id), ...input }; },
      async listReviewDecisionRecords(input) { const rows = await (await getConvexHttpClient()).query((api as any).app.listReviewDecisionRecords, { runId: input.runId as never, approvalRequestId: input.approvalRequestId as never, batchId: input.batchId as never }); return rows.map((row: any) => ({ id: String(row._id), runId: String(row.runId), workspaceId: String(row.workspaceId), systemId: row.systemId ? String(row.systemId) : undefined, approvalRequestId: row.approvalRequestId ? String(row.approvalRequestId) : undefined, batchId: row.batchId, actorId: row.actorId, decision: row.decision, note: row.note, createdAt: row.createdAt })); },
      async addApprovalParticipantRecord(input) { const row = await (await getConvexHttpClient()).mutation((api as any).app.addApprovalParticipantRecord, { ...input, approvalRequestId: input.approvalRequestId as never, runId: input.runId as never, workspaceId: input.workspaceId as never, systemId: input.systemId as never }); return { id: String(row._id), ...input }; },
      async listApprovalParticipantRecords(input) { const rows = await (await getConvexHttpClient()).query((api as any).app.listApprovalParticipantRecords, { runId: input.runId as never, approvalRequestId: input.approvalRequestId as never }); return rows.map((row: any) => ({ id: String(row._id), approvalRequestId: String(row.approvalRequestId), runId: String(row.runId), workspaceId: String(row.workspaceId), systemId: row.systemId ? String(row.systemId) : undefined, actorId: row.actorId, recommendation: row.recommendation, note: row.note, createdAt: row.createdAt })); },
      async addHandoffRecord(input) { const row = await (await getConvexHttpClient()).mutation((api as any).app.addHandoffRecord, { ...input, runId: input.runId as never, workspaceId: input.workspaceId as never, systemId: input.systemId as never, openQuestionsJson: JSON.stringify(input.openQuestions), unresolvedThreadIdsJson: JSON.stringify(input.unresolvedThreadIds) }); return { id: String(row._id), ...input }; },
      async listHandoffRecords(input) { const rows = await (await getConvexHttpClient()).query((api as any).app.listHandoffRecords, { runId: input.runId as never }); return rows.map((row: any) => ({ id: String(row._id), runId: String(row.runId), workspaceId: String(row.workspaceId), systemId: row.systemId ? String(row.systemId) : undefined, fromUserId: row.fromUserId, toUserId: row.toUserId, note: row.note, stage: row.stage, pendingApprovalCount: row.pendingApprovalCount, openQuestions: JSON.parse(row.openQuestionsJson), unresolvedThreadIds: JSON.parse(row.unresolvedThreadIdsJson), status: row.status, createdAt: row.createdAt, respondedAt: row.respondedAt })); },
      async updateHandoffStatus(input) { await (await getConvexHttpClient()).mutation((api as any).app.updateHandoffStatus, input as any); },
      async addRevisionRequest(input) { const row = await (await getConvexHttpClient()).mutation((api as any).app.addRevisionRequest, { ...input, runId: input.runId as never, workspaceId: input.workspaceId as never, systemId: input.systemId as never }); return { id: String(row._id), ...input }; },
      async listRevisionRequests(input) { const rows = await (await getConvexHttpClient()).query((api as any).app.listRevisionRequests, { runId: input.runId as never, status: input.status }); return rows.map((row: any) => ({ id: String(row._id), runId: String(row.runId), workspaceId: String(row.workspaceId), systemId: row.systemId ? String(row.systemId) : undefined, targetType: row.targetType, targetId: row.targetId, requestedBy: row.requestedBy, rationale: row.rationale, requestedOutcome: row.requestedOutcome, status: row.status, createdAt: row.createdAt, resolvedAt: row.resolvedAt })); },
      async updateRevisionRequest(input) { await (await getConvexHttpClient()).mutation((api as any).app.updateRevisionRequest, input as any); },
      async upsertAgentPolicy(input) { const row = await (await getConvexHttpClient()).mutation((api as any).app.upsertAgentPolicy, { ...input, workspaceId: input.workspaceId as never, systemId: input.systemId as never, toolJson: JSON.stringify(input.tool), riskJson: JSON.stringify(input.risk), approvalJson: JSON.stringify(input.approval), runtimeJson: JSON.stringify(input.runtime), costJson: JSON.stringify(input.cost), concurrencyJson: JSON.stringify(input.concurrency), escalationJson: JSON.stringify(input.escalation) }); return { id: String(row._id), workspaceId: String(row.workspaceId), systemId: row.systemId ? String(row.systemId) : undefined, scope: row.scope, tool: JSON.parse(row.toolJson), risk: JSON.parse(row.riskJson), approval: JSON.parse(row.approvalJson), runtime: JSON.parse(row.runtimeJson), cost: JSON.parse(row.costJson), concurrency: JSON.parse(row.concurrencyJson), escalation: JSON.parse(row.escalationJson), createdAt: row.createdAt, updatedAt: row.updatedAt }; },
      async getAgentPolicy(input) { const row = await (await getConvexHttpClient()).query((api as any).app.getAgentPolicy, { workspaceId: input.workspaceId as never, systemId: input.systemId as never }); if (!row) return null; return { id: String(row._id), workspaceId: String(row.workspaceId), systemId: row.systemId ? String(row.systemId) : undefined, scope: row.scope, tool: JSON.parse(row.toolJson), risk: JSON.parse(row.riskJson), approval: JSON.parse(row.approvalJson), runtime: JSON.parse(row.runtimeJson), cost: JSON.parse(row.costJson), concurrency: JSON.parse(row.concurrencyJson), escalation: JSON.parse(row.escalationJson), createdAt: row.createdAt, updatedAt: row.updatedAt }; },
      async addRunPolicySnapshot(input) { const row = await (await getConvexHttpClient()).mutation((api as any).app.addRunPolicySnapshot, { ...input, runId: input.runId as never, workspaceId: input.workspaceId as never, systemId: input.systemId as never, policyId: input.policyId as never, toolJson: JSON.stringify(input.tool), riskJson: JSON.stringify(input.risk), approvalJson: JSON.stringify(input.approval), runtimeJson: JSON.stringify(input.runtime), costJson: JSON.stringify(input.cost), concurrencyJson: JSON.stringify(input.concurrency), escalationJson: JSON.stringify(input.escalation) }); return { id: String(row._id), runId: String(row.runId), workspaceId: String(row.workspaceId), systemId: row.systemId ? String(row.systemId) : undefined, policyId: String(row.policyId), resolvedFromScope: row.resolvedFromScope, tool: JSON.parse(row.toolJson), risk: JSON.parse(row.riskJson), approval: JSON.parse(row.approvalJson), runtime: JSON.parse(row.runtimeJson), cost: JSON.parse(row.costJson), concurrency: JSON.parse(row.concurrencyJson), escalation: JSON.parse(row.escalationJson), createdAt: row.createdAt }; },
      async getRunPolicySnapshot(input) { const row = await (await getConvexHttpClient()).query((api as any).app.getRunPolicySnapshot, { runId: input.runId as never }); if (!row) return null; return { id: String(row._id), runId: String(row.runId), workspaceId: String(row.workspaceId), systemId: row.systemId ? String(row.systemId) : undefined, policyId: String(row.policyId), resolvedFromScope: row.resolvedFromScope, tool: JSON.parse(row.toolJson), risk: JSON.parse(row.riskJson), approval: JSON.parse(row.approvalJson), runtime: JSON.parse(row.runtimeJson), cost: JSON.parse(row.costJson), concurrency: JSON.parse(row.concurrencyJson), escalation: JSON.parse(row.escalationJson), createdAt: row.createdAt }; },
      async addPolicyDecisionRecord(input) { const row = await (await getConvexHttpClient()).mutation((api as any).app.addPolicyDecisionRecord, { ...input, runId: input.runId as never, workspaceId: input.workspaceId as never, systemId: input.systemId as never, policySnapshotId: input.policySnapshotId as never }); return { id: String(row._id), ...input }; },
      async listPolicyDecisionRecords(input) { const rows = await (await getConvexHttpClient()).query((api as any).app.listPolicyDecisionRecords, { runId: input.runId as never }); return rows.map((row: any) => ({ id: String(row._id), runId: String(row.runId), workspaceId: String(row.workspaceId), systemId: row.systemId ? String(row.systemId) : undefined, policySnapshotId: String(row.policySnapshotId), decisionType: row.decisionType, subject: row.subject, explanation: row.explanation, createdAt: row.createdAt })); },
      async upsertRuntimeUsageRecord(input) { const row = await (await getConvexHttpClient()).mutation((api as any).app.upsertRuntimeUsageRecord, { ...input, runId: input.runId as never, workspaceId: input.workspaceId as never }); return { id: String(row._id), runId: String(row.runId), workspaceId: String(row.workspaceId), providerCalls: row.providerCalls, estimatedTokens: row.estimatedTokens, estimatedCostUsd: row.estimatedCostUsd, elapsedMs: row.elapsedMs, autoAppliedActions: row.autoAppliedActions, createdAt: row.createdAt, updatedAt: row.updatedAt }; },
      async getRuntimeUsageRecord(input) { const row = await (await getConvexHttpClient()).query((api as any).app.getRuntimeUsageRecord, { runId: input.runId as never }); if (!row) return null; return { id: String(row._id), runId: String(row.runId), workspaceId: String(row.workspaceId), providerCalls: row.providerCalls, estimatedTokens: row.estimatedTokens, estimatedCostUsd: row.estimatedCostUsd, elapsedMs: row.elapsedMs, autoAppliedActions: row.autoAppliedActions, createdAt: row.createdAt, updatedAt: row.updatedAt }; },
      async addEscalationRecord(input) { const row = await (await getConvexHttpClient()).mutation((api as any).app.addEscalationRecord, { ...input, runId: input.runId as never, workspaceId: input.workspaceId as never, systemId: input.systemId as never }); return { id: String(row._id), ...input }; },
      async listEscalationRecords(input) { const rows = await (await getConvexHttpClient()).query((api as any).app.listEscalationRecords, { runId: input.runId as never }); return rows.map((row: any) => ({ id: String(row._id), runId: String(row.runId), workspaceId: String(row.workspaceId), systemId: row.systemId ? String(row.systemId) : undefined, reason: row.reason, suggestedAction: row.suggestedAction, severity: row.severity, createdAt: row.createdAt })); },
    },
    agentConversations: {
