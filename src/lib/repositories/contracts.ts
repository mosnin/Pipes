...
import type { BuilderStrategy, DecisionRecord, MemoryEntry, PatternArtifact, ReusableSubsystemPattern, SessionContinuationRef } from "@/domain/agent_builder/memory";
...
export type RepositorySet = {
  ...
  agentBuilder: {
    ...
    updateProposalBatch(input: { batchId: string; status: ProposalBatch["status"]; updatedAt?: string; reviewerNote?: string }): Promise<void>;
    ...
    addEvaluationRecord(input: Omit<EvaluationRecord, "id">): Promise<EvaluationRecord>;
    listEvaluationRecords(input: { workspaceId: string; systemId?: string; runId?: string; scope?: EvaluationRecord["scope"] }): Promise<EvaluationRecord[]>;
    addStrategyPerformanceRecord(input: Omit<StrategyPerformanceRecord, "id">): Promise<StrategyPerformanceRecord>;
    listStrategyPerformanceRecords(input: { workspaceId: string; systemId?: string; strategyId?: string; runId?: string }): Promise<StrategyPerformanceRecord[]>;
    addSkillPerformanceRecord(input: Omit<SkillPerformanceRecord, "id">): Promise<SkillPerformanceRecord>;
    listSkillPerformanceRecords(input: { workspaceId: string; systemId?: string; runId?: string; skillId?: string }): Promise<SkillPerformanceRecord[]>;
    addPatternPromotionRecord(input: Omit<PatternPromotionRecord, "id">): Promise<PatternPromotionRecord>;
    listPatternPromotionRecords(input: { workspaceId: string; systemId?: string; patternArtifactId?: string }): Promise<PatternPromotionRecord[]>;
    addPatternDemotionRecord(input: Omit<PatternDemotionRecord, "id">): Promise<PatternDemotionRecord>;
    listPatternDemotionRecords(input: { workspaceId: string; systemId?: string; patternArtifactId?: string }): Promise<PatternDemotionRecord[]>;
    addLearningArtifact(input: Omit<LearningArtifact, "id">): Promise<LearningArtifact>;
    listLearningArtifacts(input: { workspaceId: string; systemId?: string; runId?: string }): Promise<LearningArtifact[]>;
    upsertRunReviewer(input: Omit<RunReviewer, "id">): Promise<RunReviewer>;
    listRunReviewers(input: { runId: string }): Promise<RunReviewer[]>;
    upsertSharedRunVisibility(input: Omit<SharedRunVisibilityState, "id">): Promise<SharedRunVisibilityState>;
    listSharedRunVisibility(input: { runId: string }): Promise<SharedRunVisibilityState[]>;
    addReviewThread(input: Omit<ReviewThread, "id">): Promise<ReviewThread>;
    listReviewThreads(input: { runId: string; targetType?: ReviewThread["targetType"]; targetId?: string }): Promise<ReviewThread[]>;
    resolveReviewThread(input: { threadId: string; status: "resolved" | "closed"; resolvedBy: string; resolvedAt: string }): Promise<void>;
    addReviewComment(input: Omit<ReviewComment, "id">): Promise<ReviewComment>;
    listReviewComments(input: { runId: string; threadId?: string; targetType?: ReviewComment["targetType"]; targetId?: string }): Promise<ReviewComment[]>;
    addReviewDecisionRecord(input: Omit<ReviewDecisionRecord, "id">): Promise<ReviewDecisionRecord>;
    listReviewDecisionRecords(input: { runId: string; approvalRequestId?: string; batchId?: string }): Promise<ReviewDecisionRecord[]>;
    addApprovalParticipantRecord(input: Omit<ApprovalParticipantRecord, "id">): Promise<ApprovalParticipantRecord>;
    listApprovalParticipantRecords(input: { runId: string; approvalRequestId?: string }): Promise<ApprovalParticipantRecord[]>;
    addHandoffRecord(input: Omit<HandoffRecord, "id">): Promise<HandoffRecord>;
    listHandoffRecords(input: { runId: string }): Promise<HandoffRecord[]>;
    updateHandoffStatus(input: { handoffId: string; status: HandoffRecord["status"]; respondedAt?: string }): Promise<void>;
    addRevisionRequest(input: Omit<RevisionRequest, "id">): Promise<RevisionRequest>;
    listRevisionRequests(input: { runId: string; status?: RevisionRequest["status"] }): Promise<RevisionRequest[]>;
    updateRevisionRequest(input: { revisionRequestId: string; status: RevisionRequest["status"]; resolvedAt?: string }): Promise<void>;
    upsertAgentPolicy(input: Omit<AgentPolicy, "id" | "createdAt" | "updatedAt"> & { id?: string; createdAt?: string; updatedAt?: string }): Promise<AgentPolicy>;
    getAgentPolicy(input: { workspaceId: string; systemId?: string }): Promise<AgentPolicy | null>;
    addRunPolicySnapshot(input: Omit<RunPolicySnapshot, "id">): Promise<RunPolicySnapshot>;
    getRunPolicySnapshot(input: { runId: string }): Promise<RunPolicySnapshot | null>;
    addPolicyDecisionRecord(input: Omit<PolicyDecisionRecord, "id">): Promise<PolicyDecisionRecord>;
    listPolicyDecisionRecords(input: { runId: string }): Promise<PolicyDecisionRecord[]>;
    upsertRuntimeUsageRecord(input: Omit<RuntimeUsageRecord, "id" | "createdAt" | "updatedAt"> & { id?: string; createdAt?: string; updatedAt?: string }): Promise<RuntimeUsageRecord>;
  ...
  agentMemory: {
    ...
    updateMemoryEntry(input: { memoryEntryId: string; status?: MemoryEntry["status"]; confidence?: MemoryEntry["confidence"]; updatedAt?: string }): Promise<void>;
    addBuilderStrategy(input: Omit<BuilderStrategy, "id">): Promise<BuilderStrategy>;
    listBuilderStrategies(input: { workspaceId: string; systemId?: string; status?: BuilderStrategy["status"] }): Promise<BuilderStrategy[]>;
    addPatternArtifact(input: Omit<PatternArtifact, "id">): Promise<PatternArtifact>;
    listPatternArtifacts(input: { workspaceId: string; systemId?: string; status?: PatternArtifact["status"]; tag?: string }): Promise<PatternArtifact[]>;
    addReusableSubsystemPattern(input: Omit<ReusableSubsystemPattern, "id">): Promise<ReusableSubsystemPattern>;
    listReusableSubsystemPatterns(input: { workspaceId: string; systemId?: string; subsystemId?: string }): Promise<ReusableSubsystemPattern[]>;
    addDecisionRecord(input: Omit<DecisionRecord, "id">): Promise<DecisionRecord>;
    listDecisionRecords(input: { workspaceId: string; systemId?: string; runId?: string }): Promise<DecisionRecord[]>;
    addSessionContinuationRef(input: Omit<SessionContinuationRef, "id">): Promise<SessionContinuationRef>;
    listSessionContinuationRefs(input: { workspaceId: string; systemId: string; runId?: string }): Promise<SessionContinuationRef[]>;
  };
};
