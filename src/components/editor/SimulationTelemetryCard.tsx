    ...
    <ol className="flex flex-col gap-1.5 mt-2">
      {steps.map((s, i) => (
        ...
        >
          ...
          <ChevronRight size={9} className="mt-1 shrink-0 text-ink-4" />
          <div className="flex-1 min-w-0">
            <p className="t-caption text-ink-2 leading-snug" style={{ fontSize: 11 }}>{s.summary}</p>
            {s.latency_ms !== undefined && s.latency_ms > 0 && (
              <div className="mt-0.5 flex items-center gap-1.5">
                ...
                <span className="shrink-0 text-ink-3" style={{ fontSize: 9 }}>{formatMs(s.latency_ms)}</span>
              </div>
            )}
          </div>
        </motion.li>
  ...
  return (
    <div className="rounded-md border border-line bg-[#FAFAFA]">
      ...
      >
        ...
        <span className="flex-1 text-left t-caption text-ink-2" style={{ fontSize: 11 }}>
          {entry.steps.length} step{entry.steps.length !== 1 ? "s" : ""}
        </span>
        <span className="t-caption text-ink-3 flex items-center gap-0.5" style={{ fontSize: 10 }}>
          <Clock size={9} />
        ...
        <span className="t-caption text-ink-4" style={{ fontSize: 10 }}>{formatTime(entry.ranAt)}</span>
        {open ? <ChevronDown size={10} className="text-ink-3" /> : <ChevronRight size={10} className="text-ink-3" />}
      </button>
      <AnimatePresence>
        {open && (
          <motion.div
...
export function SimulationTelemetryCard({ systemId }: { systemId: string }) {
  ...
  return (
    <div className="rounded-lg border border-line surface-canvas p-3">
      <div className="flex items-center justify-between mb-2">
        <h5 className="t-label font-semibold text-ink-2">Telemetry trace</h5>
        <button
          onClick={run}
          disabled={loading}
        ...
        {result && (
          ...
          >
            ...
            <div className="flex items-center gap-3 mb-2.5 flex-wrap">
              ...
              <span className="inline-flex items-center gap-1 t-caption text-ink-2" style={{ fontSize: 11 }}>
              ...
              {result.totalTokens > 0 && (
                <>
                  ...
                  <span className="t-caption text-ink-3" style={{ fontSize: 10 }}>
      ...
      {history.length > 1 && (
        <div className="mt-3 pt-3 border-t border-line">
          <div className="flex items-center gap-1.5 mb-2">
            <History size={10} className="text-ink-3" />
            <span className="t-overline text-ink-3" style={{ fontSize: 10 }}>Past runs</span>
          </div>
