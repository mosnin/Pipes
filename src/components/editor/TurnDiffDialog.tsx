...
export function TurnDiffDialog({
  ...
  return (
    ...
    >
      {empty ? (
        <p className="t-label text-[#8E8E93] py-2">No graph changes in this turn.</p>
      ) : (
        <div className="space-y-4">
          ...
          <Section title="Updated" tone="neutral" empty={diff.nodesUpdated.length === 0}>
            {diff.nodesUpdated.map((u) => (
              <div
                key={`upd-${u.id}`}
                className="grid grid-cols-2 gap-3 border border-black/[0.06] rounded-md p-2"
              >
                <div>
                  <p className="t-caption text-[#8E8E93] mb-0.5">Before</p>
                  <p className="t-label text-[#3C3C43] truncate">{nodeLabel(u.before)}</p>
                  {u.before.description ? (
                    <p className="t-caption text-[#8E8E93] truncate">{u.before.description}</p>
                  ) : null}
                </div>
                <div>
                  <p className="t-caption text-[#8E8E93] mb-0.5">After</p>
                  <p className="t-label text-[#111] truncate">{nodeLabel(u.after)}</p>
                  {u.after.description ? (
                    <p className="t-caption text-[#3C3C43] truncate">{u.after.description}</p>
                  ) : null}
                </div>
              </div>
            ))}
...
function Section({
  ...
  return (
    <div>
      ...
      {empty ? (
        <p className="t-caption text-[#8E8E93] pl-3">Nothing.</p>
      ) : (
...
function Row({
  ...
  const color = tone === "negative" ? "text-red-600" : "text-[#3C3C43]";
  return (
    <p className={`t-label ${color} truncate`}>
      <span className="t-caption text-[#8E8E93] mr-1.5">{kind}</span>
      {text}
    </p>
  );
}
