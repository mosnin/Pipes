...
describe("enterprise auth readiness settings", () => {
  ...
  it("validates sso_ready configuration and domains", async () => {
    ...
    await expect(services.governance.updateEnterpriseAuth(ctx, { mode: "sso_ready", allowedDomains: ["example.com"], enforceDomainMatch: true })).rejects.toThrow("Auth0 connection is required");

    const updated = await services.governance.updateEnterpriseAuth(ctx, { mode: "sso_ready", allowedDomains: ["example.com"], auth0Connection: "saml-acme", enforceDomainMatch: true });
