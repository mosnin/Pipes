...
describe("agent graph action protocol", () => {
  ...
  it("approval lifecycle pauses and resumes run", async () => {
    ...
    const appliedBeforeApproval = await service.listAppliedActions(ctx as never, { runId: run.id });
    expect(appliedBeforeApproval.length).toBeGreaterThan(0);
  ...
  it("uses runtime sub-agent executor seam in live run", async () => {
    const repos = createMockRepositories();
    const service = new AgentRunService(repos);
    const systemId = (await repos.systems.list(ctx.workspaceId))[0]?.id;
    if (!systemId) throw new Error("missing seeded system");
    const session = await service.createSession(ctx as never, { systemId, title: "Executor Seam" });
    const run = (await service.createRun(ctx as never, { sessionId: session.id, systemId, message: "Improve reliability" })).run;
    await service.streamRun(ctx as never, { runId: run.id, message: "Improve reliability", systemName: "Demo" });
    const skills = await service.listSkillInvocations(ctx as never, run.id);
    expect(skills.some((row) => (row.outputSummary ?? "").includes("modal_ready_inline"))).toBe(true);
  }, 15000);
});
