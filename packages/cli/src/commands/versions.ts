...
export function registerVersions(program: Command): void {
  ...
  versions
    ...
    .action(async (systemId: string) => {
      ...
      try {
        const data = await client.get<VersionRecord[]>(
          `/api/protocol/systems/${systemId}/versions`
        );
        spinner.stop();
        if (global.json) {
          printJson(data);
  ...
  versions
    ...
    .action(async (systemId: string, opts: { name?: string; idempotencyKey?: string }) => {
      const global = program.optsWithGlobals<GlobalOpts>();
      const client = makeClient({ api: global.api, token: global.token });
      const spinner = ora("Creating version...").start();
      try {
        const ikey = opts.idempotencyKey ?? randomUUID();
        const res = await client.postRaw<{ versionId: string }>(
          `/api/protocol/systems/${systemId}/versions`,
          { name: opts.name },
          { idempotencyKey: ikey }
        );
