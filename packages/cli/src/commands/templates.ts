...
export function registerTemplates(program: Command): void {
  ...
  templates
    ...
    .action(async () => {
      ...
      try {
        const data = await client.get<TemplateRecord[]>("/api/protocol/templates");
        spinner.stop();
        if (global.json) {
          printJson(data);
          return;
  ...
  templates
    ...
    .action(async (templateId: string, opts: { name?: string; idempotencyKey?: string }) => {
      const global = program.optsWithGlobals<GlobalOpts>();
      const client = makeClient({ api: global.api, token: global.token });
      ...
      try {
        ...
        const res = await client.postRaw<{ systemId: string }>(
          `/api/protocol/templates/${templateId}/instantiate`,
          { name: opts.name },
          { idempotencyKey: ikey }
        );
        spinner.stop();
