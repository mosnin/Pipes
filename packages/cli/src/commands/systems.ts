...
export function registerSystems(program: Command): void {
  ...
  systems
    ...
    .action(async () => {
      ...
      try {
        const data = await client.get<SystemRecord[]>("/api/protocol/systems");
        spinner.stop();
        if (global.json) {
          printJson(data);
          return;
  ...
  systems
    ...
    .action(async (systemId: string) => {
      ...
      try {
        const data = await client.get<SystemRecord>(`/api/protocol/systems/${systemId}`);
        spinner.stop();
        if (global.json) {
          printJson(data);
          return;
  ...
  systems
    ...
    .action(async (name: string, opts: { description?: string; idempotencyKey?: string }) => {
      const global = program.optsWithGlobals<GlobalOpts>();
      const client = makeClient({ api: global.api, token: global.token });
