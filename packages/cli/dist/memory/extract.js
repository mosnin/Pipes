...
export async function extractMetadata(content, hints = {}) {
...
export function scoreRecord(record, queryKeywords) {
    const searchable = [
        record.title,
        record.summary,
        record.topic,
        ...
        .join(" ")
        .toLowerCase();
    const hits = queryKeywords.filter((kw) => searchable.includes(kw.toLowerCase())).length;
...
export function compressForContext(records, maxChars = 4000) {
    const lines = [];
    let chars = 0;
