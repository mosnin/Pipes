...
export function registerComments(program) {
    ...
    comments
        ...
        try {
            const payload = { systemId, body };
            if (opts.nodeId)
                payload["nodeId"] = opts.nodeId;
            const ikey = opts.idempotencyKey ?? randomUUID();
            const res = await client.postRaw("/api/protocol/comments", payload, { idempotencyKey: ikey });
            spinner.stop();
            if (!res.ok || !res.data) {
