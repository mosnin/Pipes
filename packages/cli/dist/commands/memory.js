...
function getMemorySystemId(flagValue) {
    const id = flagValue ?? process.env["PIPES_MEMORY_SYSTEM"];
    if (!id) {
        throw new Error("No memory system ID. Pass --system <id> or set PIPES_MEMORY_SYSTEM env var.");
    }
    return id;
}
function parseNodeAsRecord(node) {
    try {
        if (!node.description)
            return null;
...
export function registerMemory(program) {
    ...
    memory
        ...
        .description("Link two memory records with a relation. Relation types: supports, contradicts, derives-from, supersedes, references")
        .option("--system <id>", "Memory system ID (overrides PIPES_MEMORY_SYSTEM)")
